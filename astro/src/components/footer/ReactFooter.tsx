/**
 * ReactFooter Component
 *
 * A React-based footer component for dynamic features and interactivity.
 * This component is ready to use when dynamic functionality is needed
 * (analytics tracking, real-time updates, user interactions, etc.)
 *
 * Usage in AstroStarlightFooter.astro:
 * ```astro
 * import ReactFooter from './ReactFooter';
 * <ReactFooter client:only="react" config={config} />
 * ```
 *
 * Features:
 * - State management for dynamic content
 * - Event tracking capabilities
 * - Interactive tooltips
 * - Animation support
 * - Client-side only rendering
 */

import React, { useState, useEffect } from 'react';
import type { FooterConfig, SocialLink, FooterLink, FooterSection } from './typeFooter';

interface ReactFooterProps {
  config: FooterConfig;
  onLinkClick?: (href: string, label: string) => void;
  onSocialClick?: (platform: string, href: string) => void;
}

/**
 * Social Icon Component (React version)
 */
const SocialIcon: React.FC<{
  platform: string;
  size?: number;
  className?: string;
}> = ({ platform, size = 24, className = '' }) => {
  const icons: Record<string, string> = {
    github: 'M12 2C6.477 2 2 6.477 2 12c0 4.42 2.865 8.17 6.839 9.49.5.092.682-.217.682-.482 0-.237-.008-.866-.013-1.7-2.782.603-3.369-1.34-3.369-1.34-.454-1.156-1.11-1.463-1.11-1.463-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.831.092-.646.35-1.086.636-1.336-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.578 9.578 0 0112 6.836c.85.004 1.705.114 2.504.336 1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.203 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .267.18.578.688.48C19.138 20.167 22 16.418 22 12c0-5.523-4.477-10-10-10z',
    twitter: 'M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z',
    discord: 'M20.317 4.37a19.791 19.791 0 00-4.885-1.515.074.074 0 00-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 00-5.487 0 12.64 12.64 0 00-.617-1.25.077.077 0 00-.079-.037A19.736 19.736 0 003.677 4.37a.07.07 0 00-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 00.031.057 19.9 19.9 0 005.993 3.03.078.078 0 00.084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 00-.041-.106 13.107 13.107 0 01-1.872-.892.077.077 0 01-.008-.128 10.2 10.2 0 00.372-.292.074.074 0 01.077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 01.078.01c.12.098.246.198.373.292a.077.077 0 01-.006.127 12.299 12.299 0 01-1.873.892.077.077 0 00-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 00.084.028 19.839 19.839 0 006.002-3.03.077.077 0 00.032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 00-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z',
    linkedin: 'M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z',
    youtube: 'M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z',
    twitch: 'M11.571 4.714h1.715v5.143H11.57zm4.715 0H18v5.143h-1.714zM6 0L1.714 4.286v15.428h5.143V24l4.286-4.286h3.428L22.286 12V0zm14.571 11.143l-3.428 3.428h-3.429l-3 3v-3H6.857V1.714h13.714z',
    mastodon: 'M23.268 5.313c-.35-2.578-2.617-4.61-5.304-5.004C17.51.242 15.792 0 11.813 0h-.03c-3.98 0-4.835.242-5.288.309C3.882.692 1.496 2.518.917 5.127.64 6.412.61 7.837.661 9.143c.074 1.874.088 3.745.26 5.611.118 1.24.325 2.47.62 3.68.55 2.237 2.777 4.098 4.96 4.857 2.336.792 4.849.923 7.256.38.265-.061.527-.132.786-.213.585-.184 1.27-.39 1.774-.753a.057.057 0 00.023-.043v-1.809a.052.052 0 00-.02-.041.053.053 0 00-.046-.01 20.282 20.282 0 01-4.709.545c-2.73 0-3.463-1.284-3.674-1.818a5.593 5.593 0 01-.319-1.433.053.053 0 01.066-.054c1.517.363 3.072.546 4.632.546.376 0 .75 0 1.125-.01 1.57-.044 3.224-.124 4.768-.422.038-.008.077-.015.11-.024 2.435-.464 4.753-1.92 4.989-5.604.008-.145.03-1.52.03-1.67.002-.512.167-3.63-.024-5.545zm-3.748 9.195h-2.561V8.29c0-1.309-.55-1.976-1.67-1.976-1.23 0-1.846.79-1.846 2.35v3.403h-2.546V8.663c0-1.56-.617-2.35-1.848-2.35-1.112 0-1.668.668-1.67 1.977v6.218H4.822V8.102c0-1.31.337-2.35 1.011-3.12.696-.77 1.608-1.164 2.74-1.164 1.311 0 2.302.5 2.962 1.498l.638 1.06.638-1.06c.66-.999 1.65-1.498 2.96-1.498 1.13 0 2.043.395 2.74 1.164.675.77 1.012 1.81 1.012 3.12z',
    reddit: 'M12 0A12 12 0 000 12a12 12 0 0012 12 12 12 0 0012-12A12 12 0 0012 0zm5.01 4.744c.688 0 1.25.561 1.25 1.249a1.25 1.25 0 01-2.498.056l-2.597-.547-.8 3.747c1.824.07 3.48.632 4.674 1.488.308-.309.73-.491 1.207-.491.968 0 1.754.786 1.754 1.754 0 .716-.435 1.333-1.01 1.614a3.111 3.111 0 01.042.52c0 2.694-3.13 4.87-7.004 4.87-3.874 0-7.004-2.176-7.004-4.87 0-.183.015-.366.043-.534A1.748 1.748 0 014.028 12c0-.968.786-1.754 1.754-1.754.463 0 .898.196 1.207.49 1.207-.883 2.878-1.43 4.744-1.487l.885-4.182a.342.342 0 01.14-.197.35.35 0 01.238-.042l2.906.617a1.214 1.214 0 011.108-.701zM9.25 12C8.561 12 8 12.562 8 13.25c0 .687.561 1.248 1.25 1.248.687 0 1.248-.561 1.248-1.249 0-.688-.561-1.249-1.249-1.249zm5.5 0c-.687 0-1.248.561-1.248 1.25 0 .687.561 1.248 1.249 1.248.688 0 1.249-.561 1.249-1.249 0-.687-.562-1.249-1.25-1.249zm-5.466 3.99a.327.327 0 00-.231.094.33.33 0 000 .463c.842.842 2.484.913 2.961.913.477 0 2.105-.056 2.961-.913a.361.361 0 00.029-.463.33.33 0 00-.464 0c-.547.533-1.684.73-2.512.73-.828 0-1.979-.196-2.512-.73a.326.326 0 00-.232-.095z',
  };

  const iconPath = icons[platform.toLowerCase()] || icons.github;

  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      xmlns="http://www.w3.org/2000/svg"
      className={`social-icon ${className}`}
      aria-hidden="true"
    >
      <path fill="currentColor" d={iconPath} />
    </svg>
  );
};

/**
 * Tooltip Component
 */
const Tooltip: React.FC<{
  content: string;
  children: React.ReactNode;
  position?: 'top' | 'bottom' | 'left' | 'right';
}> = ({ content, children, position = 'top' }) => {
  const [isVisible, setIsVisible] = useState(false);

  return (
    <div
      className="tooltip-wrapper"
      onMouseEnter={() => setIsVisible(true)}
      onMouseLeave={() => setIsVisible(false)}
      onFocus={() => setIsVisible(true)}
      onBlur={() => setIsVisible(false)}
    >
      {children}
      {isVisible && (
        <span className={`tooltip tooltip-${position}`} role="tooltip">
          {content}
        </span>
      )}
    </div>
  );
};

/**
 * Main React Footer Component
 */
export const ReactFooter: React.FC<ReactFooterProps> = ({
  config,
  onLinkClick,
  onSocialClick,
}) => {
  const [mounted, setMounted] = useState(false);

  useEffect(() => {
    setMounted(true);
  }, []);

  if (!mounted) {
    return null; // Prevent hydration issues
  }

  const getSocialLabel = (platform: string, customLabel?: string): string => {
    return customLabel || `${platform.charAt(0).toUpperCase() + platform.slice(1)}`;
  };

  const getSocialTooltip = (social: SocialLink): string => {
    return social.tooltip || `Follow us on ${getSocialLabel(social.platform, social.label)}`;
  };

  const handleLinkClick = (href: string, label: string) => {
    if (onLinkClick) {
      onLinkClick(href, label);
    }
  };

  const handleSocialClick = (platform: string, href: string) => {
    if (onSocialClick) {
      onSocialClick(platform, href);
    }
  };

  const layoutClass = config.layout ? `layout-${config.layout}` : 'layout-default';
  const borderClass = config.showBorder ? 'with-border' : 'no-border';
  const showTooltips = config.showTooltips !== false;

  return (
    <footer
      className={`react-footer sl-flex custom-footer ${layoutClass} ${borderClass} ${config.className || ''}`}
      role="contentinfo"
      aria-label="Site footer"
      itemScope
      itemType="https://schema.org/WPFooter"
    >
      {/* Footer Sections */}
      {config.sections && config.sections.length > 0 && (
        <div className="footer-sections" role="group" aria-label="Footer sections">
          {config.sections.map((section, sectionIdx) => (
            <nav
              key={sectionIdx}
              className="footer-section"
              aria-labelledby={`react-section-${section.title.toLowerCase().replace(/\s+/g, '-')}`}
              itemScope
              itemType="https://schema.org/SiteNavigationElement"
            >
              <h3
                className="section-title"
                id={`react-section-${section.title.toLowerCase().replace(/\s+/g, '-')}`}
                itemProp="name"
              >
                {section.title}
              </h3>
              <ul className="section-links" role="list">
                {section.links.map((link, linkIdx) => (
                  <li key={linkIdx}>
                    {showTooltips && link.tooltip ? (
                      <Tooltip content={link.tooltip}>
                        <a
                          href={link.href}
                          target={link.external ? '_blank' : undefined}
                          rel={link.external ? 'noopener noreferrer nofollow' : undefined}
                          aria-label={link.ariaLabel || link.label}
                          className="section-link"
                          itemProp="url"
                          onClick={() => handleLinkClick(link.href, link.label)}
                        >
                          <span itemProp="name">{link.label}</span>
                          {link.external && (
                            <span className="external-indicator" aria-hidden="true">
                              {' '}
                              ↗
                            </span>
                          )}
                        </a>
                      </Tooltip>
                    ) : (
                      <a
                        href={link.href}
                        target={link.external ? '_blank' : undefined}
                        rel={link.external ? 'noopener noreferrer nofollow' : undefined}
                        aria-label={link.ariaLabel || link.label}
                        className="section-link"
                        itemProp="url"
                        onClick={() => handleLinkClick(link.href, link.label)}
                      >
                        <span itemProp="name">{link.label}</span>
                        {link.external && (
                          <span className="external-indicator" aria-hidden="true">
                            {' '}
                            ↗
                          </span>
                        )}
                      </a>
                    )}
                  </li>
                ))}
              </ul>
            </nav>
          ))}
        </div>
      )}

      {/* Custom Links */}
      {config.customLinks && config.customLinks.length > 0 && (
        <nav
          className="custom-links sl-flex"
          aria-label="Footer navigation links"
          itemScope
          itemType="https://schema.org/SiteNavigationElement"
        >
          {config.customLinks.map((link, idx) => {
            const LinkContent = (
              <a
                href={link.href}
                target={link.external ? '_blank' : undefined}
                rel={link.external ? 'noopener noreferrer nofollow' : undefined}
                aria-label={link.ariaLabel || link.label}
                className="footer-link"
                itemProp="url"
                onClick={() => handleLinkClick(link.href, link.label)}
              >
                <span itemProp="name">{link.label}</span>
                {link.external && (
                  <span className="external-indicator" aria-hidden="true">
                    {' '}
                    ↗
                  </span>
                )}
              </a>
            );

            return showTooltips && link.tooltip ? (
              <Tooltip key={idx} content={link.tooltip}>
                {LinkContent}
              </Tooltip>
            ) : (
              <div key={idx}>{LinkContent}</div>
            );
          })}
        </nav>
      )}

      {/* Social Links */}
      {config.socialLinks && config.socialLinks.length > 0 && (
        <nav className="social-links sl-flex" aria-label="Social media links">
          {config.socialLinks.map((social, idx) => {
            const SocialLinkContent = (
              <a
                href={social.href}
                target="_blank"
                rel="noopener noreferrer nofollow me"
                aria-label={social.ariaLabel || `Follow us on ${getSocialLabel(social.platform, social.label)}`}
                className="social-link"
                itemProp="url"
                onClick={() => handleSocialClick(social.platform, social.href)}
              >
                <SocialIcon platform={social.platform} size={24} />
                <span className="visually-hidden" itemProp="name">
                  {getSocialLabel(social.platform, social.label)}
                </span>
              </a>
            );

            return showTooltips ? (
              <Tooltip key={idx} content={getSocialTooltip(social)} position="top">
                {SocialLinkContent}
              </Tooltip>
            ) : (
              <div key={idx}>{SocialLinkContent}</div>
            );
          })}
        </nav>
      )}

      {/* Copyright */}
      {config.copyright && (
        <div className="copyright" aria-label="Copyright information">
          <p itemProp="copyrightNotice">{config.copyright}</p>
        </div>
      )}

      {/* Starlight Credit */}
      {config.showStarlightCredit && (
        <a
          className="kudos sl-flex"
          href="https://starlight.astro.build"
          target="_blank"
          rel="noopener noreferrer nofollow"
          aria-label="Built with Starlight documentation framework - Opens in new tab"
        >
          <span className="starlight-icon" aria-hidden="true">
            🌟
          </span>
          <span>Built with Starlight</span>
        </a>
      )}
    </footer>
  );
};

export default ReactFooter;
